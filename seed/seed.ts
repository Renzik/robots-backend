...
const seedData = async () => {
  ...
  const projectsBatch = [
    ...
    },
    {
      ...
      priority: 5,
      completed: false,
      description: 'Try to have as much fun as possible on the way, regret is not an option.',
    },
    {
      ...
      priority: 9,
      completed: false,
      description: 'Finish this shit so we can start working on dope platforms as projects.',
    },
  ];

  try {
    const robots = await Robot.collection.insertMany(robotsBatch);
    console.log(':D Robots Seeded Successfully', robots);
    const projects = await Project.collection.insertMany(projectsBatch);
    console.log(':D Projects Seeded Successfully', projects);
  } catch (error) {
